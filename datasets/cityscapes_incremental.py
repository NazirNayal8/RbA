...
class CityscapesIncremental(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, hparams, split='train', target_type='semantic', transform=None, 
    mode='gtFine', hide_classes=[], class_offsets=[0]*20):
        ...
        self.class_shifts = np.zeros(20)
        for i in range(20):
            for c in self.hide_classes:
                if i > c:
                    self.class_shifts[i] += 1
            self.class_shifts[i] -= self.class_offsets[i]
    ...
    def __getitem__(self, index):
        ...
        #     if (target == i).sum() == 0:
        #         continue
        #     target[target == i] = int(i + 3)
        #     print("check")
        #     #print(i, self.class_shifts[i], i, i - self.class_shifts[i])
        # for i in [16, 17, 18]:
        #     if (target == i).sum() == 0:
        #         continue
        #     target[target == i] = int(i - 3)
        target[target == 19] = 22
        target[target >= 13] = target[target >= 13] + 3
        target[target >= 19] = target[target >= 19] - 6
