...
class D2MViT(MViT):
    ...
    def __init__(self, cfg, input_shape):
        ...
        img_size = cfg.MODEL.MVIT.IMG_SIZE
        patch_kernel = cfg.MODEL.MVIT.PATCH_KERNEL
        patch_stride = cfg.MODEL.MVIT.PATCH_STRIDE
        patch_padding = cfg.MODEL.MVIT.PATCH_PADDING
        in_chans = cfg.MODEL.MVIT.IN_CHANS
        embed_dim =cfg.MODEL.MVIT.EMBED_DIM
        depth = cfg.MODEL.MVIT.DEPTH
        num_heads = cfg.MODEL.MVIT.NUM_HEADS
        last_block_indexes = cfg.MODEL.MVIT.LAST_BLOCK_INDEXES
        qkv_pool_kernel = cfg.MODEL.MVIT.QKV_POOL_KERNEL
        adaptive_kv_stride = cfg.MODEL.MVIT.ADAPTIVE_KV_STRIDE
        adaptive_window_size = cfg.MODEL.MVIT.ADAPTIVE_WINDOW_SIZE
        residual_pooling = cfg.MODEL.MVIT.RESIDUAL_POOLING
        mlp_ratio = cfg.MODEL.MVIT.MLP_RATIO
        qkv_bias = cfg.MODEL.MVIT.QKV_BIAS
        drop_path_rate = cfg.MODEL.MVIT.DROP_PATH_RATE
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        act_layer = nn.GELU
        use_abs_pos = cfg.MODEL.MVIT.USE_ABS_POS
        use_rel_pos = cfg.MODEL.MVIT.USE_REL_POS
        rel_pos_zero_init = cfg.MODEL.MVIT.REL_POS_ZERO_INIT
        use_act_checkpoint = cfg.MODEL.MVIT.USE_ACT_CHECKPOINT
        pretrain_img_size = cfg.MODEL.MVIT.PRETRAIN_IMG_SIZE
        pretrain_use_cls_token = cfg.MODEL.MVIT.PRETRAIN_USE_CLS_TOKEN
        out_features = cfg.MODEL.MVIT.OUT_FEATURES
